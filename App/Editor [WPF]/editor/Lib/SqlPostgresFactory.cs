...
namespace Lib
{
    public class SqlPostgresFactory : IEntityFactory
    {
        ...
        public SqlPostgresFactory(IModel model)
        {
            this.model = model;
        }
        ...
        public IEntityPersistent QueryEntity<T>(string query) where T : IEntityPersistent, new()
        {
            ...
            NpgsqlDataReader reader;
            NpgsqlConnection conn = GetConnection();//nouvelle connexion pour une possible imbrication
            NpgsqlCommand cmd = new NpgsqlCommand();
            cmd.CommandTimeout = this.CommandTimeout;
